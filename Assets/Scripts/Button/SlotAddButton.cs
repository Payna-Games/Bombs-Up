...
public class SlotAddButton : MonoBehaviour
{
    ...
    public void ObjectLocalize()
    {
        ...
        {
            if (item.GetComponent<GridIsEmpty>().gridObject == null && SceneManager.GetActiveScene().name == "_Level-Tutorial" && transform.GetComponent<EnoughMoney>().clickCount <=2)
            {
                currentObjectType = 2;
                ObjectType(item);
                break;
            }
            else if (item.GetComponent<GridIsEmpty>().gridObject == null && transform.GetComponent<EnoughMoney>().clickCount > 2)
            {
